...
Description:
Map2 ROS2 bag replay of end-to-end inference pipeline for LiDAR-based ConvMLP pose regression including feature extraction, scaling, ConvMLP prediction,and recursive EKF-based smoothing. 
...
PYTORCH_MODEL_PATH = os.environ.get("PYTORCH_MODEL_PATH", "models/ConvMLP_Best_L2.pt")
...
class LocalizationInferenceLive(Node):
    ...
    def _update_plot(self):
        ...
        self.ax.clear()
        self.ax.plot([-0.17, 1.3], [0.72, 0.68], 'k-', linewidth=3, label='Map Boundary') 
        # Vertical inner wall
        self.ax.plot([1.3, 1.3], [0.68, 2.4], 'k-', linewidth=3)
        # Bottom wall
        self.ax.plot([-0.17,2.06], [-0.10, -0.10], 'k-', linewidth=3)
        # Right vertical wall
        self.ax.plot([2.06, 2.06], [-0.10, 2.4], 'k-', linewidth=3)
        # Left closing wall
        self.ax.plot([-0.17, -0.17], [-0.10, 0.72], 'k-', linewidth=2)
        # Top closing wall
        self.ax.plot([1.3, 2.06], [2.4, 2.4], 'k-', linewidth=2)
    
        # --- Obstacles ---
        # Obstacle 1
        obs1 = patches.Rectangle((1.32, -0.10), 0.2, 0.17, linewidth=1, 
                             edgecolor='r', facecolor='gray', alpha=0.5, label='Obstacle 1')
        self.ax.add_patch(obs1)
    
        # Obstacle 2
        obs2 = patches.Rectangle((1.30, 1.33),0.2, 0.2, linewidth=1, 
                             edgecolor='r', facecolor='black', alpha=0.5, label = 'Obstacle 2')
        self.ax.add_patch(obs2)
    
        # Obstacle 3
        obs3 = patches.Rectangle((1.70, 2.32), 0.1, 0.08, linewidth=1, 
                             edgecolor='r', facecolor='brown', alpha=0.5, label = 'Obstacle 3')
