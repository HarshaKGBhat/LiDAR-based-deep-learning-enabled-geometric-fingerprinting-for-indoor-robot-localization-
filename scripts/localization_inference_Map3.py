...
Description:
Map3 ROS2 bag replay of end-to-end inference pipeline for LiDAR-based ConvMLP pose regression including feature extraction, scaling, ConvMLP prediction,and recursive EKF-based smoothing. 
...
PYTORCH_MODEL_PATH = os.environ.get("PYTORCH_MODEL_PATH", "models/ConvMLP_Best_3.pt")
...
class LocalizationInferenceLive(Node):
    ...
    # --------------------------
    # SCAN + TF → get SLAM GT & run inference Note: we are using the SLAM GT just to compare the ML result. The developed method does not have any connection with SLAM GT to predict the localization.
    # --------------------------
    ...
    def _update_plot(self):
        ...
        self.ax.clear()
        # Drawing Map
        self.ax.plot([-0.4, 6], [-0.4, -0.4], 'k-', linewidth=3) # Bottom
        self.ax.plot([6, 6], [-0.4, 6], 'k-', linewidth=3)       # Right
        self.ax.plot([6, -0.4], [6, 6], 'k-', linewidth=3)       # Top
        self.ax.plot([-0.4, -0.4], [6, -0.4], 'k-', linewidth=3) # Left

    # 2. Centered L-Shaped House
    # Shift parameters: X_shift = +2.0, Y_shift = +2.0
        shift_x = 1.0
        shift_y = 2.0
    
    # Horizontal walls of the L
        self.ax.plot([0 + shift_x, 1.2 + shift_x], [0.5 + shift_y, 0.5 + shift_y], 'k-', lw=2) # Inner horizontal
        self.ax.plot([0 + shift_x, 2.2 + shift_x], [-0.5 + shift_y, -0.5 + shift_y], 'k-', lw=2) # Bottom L
        self.ax.plot([1.2 + shift_x, 2.2 + shift_x], [2.6 + shift_y, 2.6 + shift_y], 'k-', lw=1) # Top L
    
    # Vertical walls of the L
        self.ax.plot([1.2 + shift_x, 1.2 + shift_x], [0.5 + shift_y, 2.6 + shift_y], 'k-', lw=2) # Inner Vertical
        self.ax.plot([2.2 + shift_x, 2.2 + shift_x], [-0.5 + shift_y, 2.6 + shift_y], 'k-', lw=2) # Right Vertical
        self.ax.plot([0 + shift_x, 0 + shift_x], [-0.5 + shift_y, 0.5 + shift_y], 'k-', lw=1) # Left Cap
        import matplotlib.patches as patches

        obs1 = patches.Rectangle((-0.4, 0.9), 0.2, 1.5, lw=1, edgecolor='r', facecolor='gray',    alpha=0.5, label = 'Obstacle 5')
        self.ax.add_patch(obs1)
    

        obs2 = patches.Rectangle((4.2, 0.5), 1.8, 1.0, lw=1, edgecolor='r', facecolor='black', alpha=0.5, label = 'Obstacle 2')
        self.ax.add_patch(obs2)
    
    
        obs5 = patches.Rectangle((5.5, 2.0), 0.4, 0.4, lw=1, edgecolor='r', facecolor='brown', alpha=0.5, label = 'Door')
        self.ax.add_patch(obs5)
    
    
        obs4 = patches.Rectangle((4.3, -0.2), 1.7, 0.6, lw=1, edgecolor='r', facecolor='green', alpha=0.5, label = 'Obstacle 1')
        self.ax.add_patch(obs4)

    # ... (existing obs4 code) ...
        obs3 = patches.Rectangle((1.0, 4.2), 1.1, 0.6, lw=1, edgecolor='r', facecolor='red', alpha=0.5, label = 'Obstacle 3')
